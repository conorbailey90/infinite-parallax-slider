...
items.forEach((item,idx) => {
    item.style.backgroundImage = `url(./images/${idx + 1}.jpeg)`
})
...
sliderParent.addEventListener('wheel', (e) => {
    ...
    if(value >= 0){value = -(slider.getBoundingClientRect().width / 2) + 30}
    if(value <= -(slider.getBoundingClientRect().width / 2) + 29){value = 1}
...
function animate(){
    for(let i = 0; i < items.length; i++){
        let {left} = items[i].parentElement.getBoundingClientRect();
        items[i].style.transform = `translate3D(${((left - ((window.innerWidth / 2) - 150)) * 0.2) * -1}px, 0, 0)`
    }
